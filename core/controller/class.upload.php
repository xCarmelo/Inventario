...
class upload {
    ...
    function getsize($size) {
        if (!is_numeric($size)) {
            ...
            $size = (int) $size; // Convertir la parte numérica inicial del tamaño a entero
            switch ($last) {
                case 'g':
                    $size *= 1024 * 1024 * 1024;
                    break;
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->image_convert && $this->file_is_image) { // si convertimos como una imagen
                if ($this->file_src_name_ext) {
                    $this->file_dst_name_ext = $this->image_convert;
                    $this->log .= '- nueva extensión de nombre de archivo: ' . $this->image_convert . '<br />';
                }
            }
            
            if (!is_null($this->file_new_name_body)) { // renombrar el cuerpo del archivo
                $this->file_dst_name_body = $this->file_new_name_body;
                $this->log .= '- nuevo cuerpo de nombre de archivo: ' . $this->file_new_name_body . '<br />';
            }
            
            if (!is_null($this->file_new_name_ext)) { // renombrar la extensión del archivo
                $this->file_dst_name_ext = $this->file_new_name_ext;
                $this->log .= '- nueva extensión de nombre de archivo: ' . $this->file_new_name_ext . '<br />';
            }
            
            if (!is_null($this->file_name_body_add)) { // agregar una cadena al nombre
                $this->file_dst_name_body .= $this->file_name_body_add;
                $this->log .= '- agregar al cuerpo del nombre de archivo: ' . $this->file_name_body_add . '<br />';
            }
            
            if (!is_null($this->file_name_body_pre)) { // agregar una cadena al principio del nombre
                $this->file_dst_name_body = $this->file_name_body_pre . $this->file_dst_name_body;
                $this->log .= '- agregar al principio del cuerpo del nombre de archivo: ' . $this->file_name_body_pre . '<br />';
            }
            ...
            if ($this->file_safe_name) {
                ...
                // Convertir a UTF-8
                $this->file_dst_name_body = mb_convert_encoding($this->file_dst_name_body, 'UTF-8', mb_detect_encoding($this->file_dst_name_body));
            
                // Reemplazar caracteres no válidos
                $this->file_dst_name_body = preg_replace(array('/\s/', '/\.[\.]+/', '/[^\w_\.\-]/'), array('_', '.', ''), $this->file_dst_name_body);
            
                $this->log .= '- formato seguro del nombre de archivo<br />';
            }
